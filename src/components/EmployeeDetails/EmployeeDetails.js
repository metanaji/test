  ...
  const columns = [
    {
      ...
      grow: 0,
      width: '13%',
      cell: row => { if (row.avatar === "http://httpstat.us/503" || row.avatar === "0") { return "Avatar Not Available" } else { return <img onClick={() => showFullAvtar(row)} width="56px" alt={row.name} src={row.avatar} /> } },
    },
    {
      name: 'Name',
      selector: 'name',
